Nano_Suit_ONI ={
  ...
  MEH_HUD = addMissionEventHandler ["EachFrame", {
    ...
    //DEBUG
    //nanite_new_UI_ONI = true;
    //nanite_GUI_HidePercentage_ONI = false;

    if (nanite_new_UI_ONI) then {
      ...
      _set_bar progressSetPosition (SuitPower_ONI/100);

      if (!nanite_GUI_HidePercentage_ONI) then {
    ...
    if (SuitPower_ONI > 0) then {
      ...
      if (ArmorActive == 0) then {
        if (player == vehicle player) then {
          ...
          if (eta_vehicle_shield_ONI) then
          {
            ...
            shield_power_drain = eta_vehicle_shield_power_drain_ONI;
  ...
  Nano_STOP = {
    ...
    [] spawn {
      ...
      sleep eta_cloak_cooldown_timer_ONI;
      player setVariable ["_eta_cloak_cooldown", true, true];
    };
    {
      if (_x isKindOf "OPTRE_FC_Energy_shield") then {
