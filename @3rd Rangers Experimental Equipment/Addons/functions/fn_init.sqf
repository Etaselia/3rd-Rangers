  ...
  ["itemAdd", ["cats_assault", {
    ...
    EH_Draw_Icon = addMissionEventHandler["EachFrame",{
      if (count ShotLocation != 0) then {
        drawIcon3D ['A3\ui_f_curator\Data\CfgCurator\entity_disabled_ca.paa', [1,0,0,0.5], ShotLocation, 1, 1, 0,"Shot detected!", 2, 0.05, 'RobotoCondensed', 'center', true];
        if !(oldShot isEqualTo ShotLocation) then {
          hintSilent formatText ["New Shot Detected"];
          oldShot = ShotLocation;
          [5,false] call BIS_fnc_countDown;
        }
        else {
          time_left = [0] call BIS_fnc_countdown;;
          if (time_left == 0) then {
            [-1] call BIS_fnc_countdown;
            hintSilent "Time UP";
            drawIcon3D ['A3\ui_f_curator\Data\CfgCurator\entity_disabled_ca.paa', [1,0,0,0.5], getPosATL player, 1, 1, 0,"C.A.T.S. Shutdown", 2, 0.05, 'RobotoCondensed', 'center', false];
            removeMissionEventHandler ["EachFrame", EH_Draw_Icon];
          };
        };
      };
    }];
