waitUntil { not isNull player };
  ...
  VehiclePunch_fnc ={
    ...
    if ([_current_uniform, punch_uniforms] call BIS_fnc_inString or all_can_punch) then {
      _target = cursorTarget;
      if ((player distance _target <= max_punch_distance and _target call BIS_fnc_objectType select 0 in ["Vehicle","Soldier"])) then {

      [player, punch_animation] remoteExec ["switchMove", 0];
